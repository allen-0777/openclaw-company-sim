...
import { TileType, EditTool, Direction } from '@/lib/pixel-office/types'
...
export default function PixelOfficePage() {
  ...
  const { runAgentTask } = useOfficeWebSocket({
    onMessage: (msg) => {
      if (msg.type === 'agent_log' && bossPanelAgent && msg.agent_name === bossPanelAgent.id) {
        setBossPanelLogs(prev => [...prev, msg.line])
      }
    }
  })
  ...
  useEffect(() => {
    const handleInteraction = () => {
      const office = officeRef.current
      if (!office || !office.player) return

      const p = office.player
      // Check 1 tile in front of player
      let tx = p.x
      let ty = p.y
      // Use a slightly larger offset to ensure we hit the target tile center
      const offset = TILE_SIZE
      if (p.dir === Direction.LEFT) tx -= offset
      else if (p.dir === Direction.RIGHT) tx += offset
      else if (p.dir === Direction.UP) ty -= offset
      else if (p.dir === Direction.DOWN) ty += offset

      const tileX = Math.floor(tx / TILE_SIZE)
      const tileY = Math.floor(ty / TILE_SIZE)

      // 1. Check characters
      const charId = office.getCharacterAt(tx, ty)
      if (charId !== null) {
        // Find agent ID
        for (const [aid, cid] of agentIdMapRef.current.entries()) {
          if (cid === charId) {
            // Open Boss Interaction Panel instead of detailed stats
            // setSelectedAgentId(aid)
            ...
            if (agent) {
              ...
              const ch = office.characters.get(cid)
              if (ch) {
                // Opposite of player dir
                if (p.dir === Direction.UP) ch.dir = Direction.DOWN
                else if (p.dir === Direction.DOWN) ch.dir = Direction.UP
                else if (p.dir === Direction.LEFT) ch.dir = Direction.RIGHT
                else if (p.dir === Direction.RIGHT) ch.dir = Direction.LEFT
              }
            }
            return
          }
        ...
        if (ch?.isSubagent && ch.parentAgentId != null) {
          // ... handle subagent click ...
        }
        return
      }

      // 2. Check furniture interactions
      // PC -> Gateway Chat
      const onPC = office.layout.furniture.some(f => {
        if (f.type !== 'pc') return false
        const entry = getCatalogEntry(f.type)
    ...
    const handlePlayerKeyDown = (e: KeyboardEvent) => {
      const office = officeRef.current
      if (!office || isEditMode) return // Disable walking while in edit mode
      ...
      switch (e.key.toLowerCase()) {
        case 'w':
        case 'arrowup':
          office.movePlayer(Direction.UP)
          break
        case 'a':
        case 'arrowleft':
          office.movePlayer(Direction.LEFT)
          break
        case 's':
        case 'arrowdown':
          office.movePlayer(Direction.DOWN)
          break
        case 'd':
        case 'arrowright':
          office.movePlayer(Direction.RIGHT)
          break
        case ' ':
        case 'enter':
          handleInteraction()
    ...
    const handlePlayerKeyUp = (e: KeyboardEvent) => {
      const office = officeRef.current
      if (!office || isEditMode) return
    ...
    try {
      const res = await fetch('/api/test-session', {
        ...
        body: JSON.stringify({
          ...
          sessionKey: `agent:${bossPanelAgent.id}:main`,
          text: cmd,
        })
      })
      const data = await res.json()
      ...
      setBossPanelMessages(prev => [...prev, { role: 'agent', content: `❌ Network error: ${e.message}` }])
    }
  }

  return (
    <div className="relative flex flex-col overflow-hidden h-[calc(100dvh-3.5rem)] md:h-full">
      {/* Boss Interaction Panel */}
      {bossPanelAgent && (
        <BossInteractionPanel
          agentId={bossPanelAgent.id}
          agentName={bossPanelAgent.name}
          agentEmoji={bossPanelAgent.emoji}
          onClose={() => {
            setBossPanelAgent(null)
            setBossPanelMessages([])
            setBossPanelLogs([])
