...
// Extend type
let cachedOpenclawVersion: { value: string | null; cliPath: string | null; expiresAt: number } | null = null;
...
async function getOpenclawVersion(): Promise<{ version: string | undefined; cliPath: string | undefined }> {
  const now = Date.now();
  if (cachedOpenclawVersion && cachedOpenclawVersion.expiresAt > now) {
    return { version: cachedOpenclawVersion.value || undefined, cliPath: cachedOpenclawVersion.cliPath || undefined };
  }
  try {
    const { stdout } = await execOpenclaw(["--version"]);
    const version = stdout.trim().split(/\s+/)[0] || null;
    const pathOutput = await execOpenclaw(["which", "openclaw"]).catch(() => ({ stdout: "" }));
    const cliPath = pathOutput.stdout.trim() || null;
    
    cachedOpenclawVersion = { value: version, cliPath, expiresAt: now + 60 * 60 * 1000 };
    return { version: version || undefined, cliPath: cliPath || undefined };
  } catch {
    cachedOpenclawVersion = { value: null, cliPath: null, expiresAt: now + 60 * 1000 };
    return { version: undefined, cliPath: undefined };
  }
}
...
export async function GET() {
  ...
  try {
    const { version: openclawVersion, cliPath } = await getOpenclawVersion();
