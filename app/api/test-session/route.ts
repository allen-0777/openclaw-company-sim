...
export async function POST(req: Request) {
  try {
    const { sessionKey, agentId, text } = await req.json();
    if (!sessionKey || !agentId) {
      return NextResponse.json({ error: "Missing sessionKey or agentId" }, { status: 400 });
    }
    const messageContent = typeof text === "string" && text.trim()
      ? text.trim()
    ...
    try {
      ...
      return NextResponse.json({
        ...
        sessionKey,
        elapsed,
        reply: text ? reply : reply.slice(0, 200) || "(no reply)",
      });
    } catch (err: any) {
      const elapsed = Date.now() - startTime;
