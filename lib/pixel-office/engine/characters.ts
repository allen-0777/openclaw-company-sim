...
export function updatePlayer(
  ...
  if (moveDir !== null) {
    ...
    ch.frame = 1 // Idle frame
    ch.frameTimer = 0
  }

  if (moveDir === null) return

  // Move faster than AI (e.g. 1.8x)
  const speed = WALK_SPEED_PX_PER_SEC * 1.8
  ...
  if (moveDir === Direction.LEFT) nextX -= moveDist
  else if (moveDir === Direction.RIGHT) nextX += moveDist
  else if (moveDir === Direction.UP) nextY -= moveDist
  else if (moveDir === Direction.DOWN) nextY += moveDist

  // Collision check: bounding box (center +/- 6px)
  const margin = 6
  // Check corners
  const left = Math.floor((nextX - margin) / TILE_SIZE)
  const right = Math.floor((nextX + margin) / TILE_SIZE)
  const top = Math.floor((nextY - margin) / TILE_SIZE)
  const bottom = Math.floor((nextY + margin) / TILE_SIZE)

  const tl = isWalkable(left, top, tileMap, blockedTiles)
  const tr = isWalkable(right, top, tileMap, blockedTiles)
  const bl = isWalkable(left, bottom, tileMap, blockedTiles)
  const br = isWalkable(right, bottom, tileMap, blockedTiles)

  if (tl && tr && bl && br) {
    ch.x = nextX
    ch.y = nextY
    ch.tileCol = Math.floor(nextX / TILE_SIZE)
    ch.tileRow = Math.floor(nextY / TILE_SIZE)
    ch.dir = moveDir
  } else {
    // Basic sliding: try moving only along one axis if blocked diagonally?
    // But we only move cardinally here, so if blocked, just stop.
    // Or try to slide if slightly offset?
    // For now, simple stop.
  }
}
