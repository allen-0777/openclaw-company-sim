...
import { createCharacter, updateCharacter, updatePlayer } from './characters'
...
export class OfficeState {
  ...
  private playerMoveDir: Direction | null = null
  ...
  spawnPlayer(col?: number, row?: number): Character {
    const id = 0 // Player ID
    if (this.characters.has(id)) this.characters.delete(id)
    const palette = 2 // Green-ish palette
    const hueShift = 0
    const ch = createCharacter(id, palette, null, null, hueShift)
    ch.isPlayer = true
    ch.label = 'YOU'
    
    // Choose spawn point: doorway or center bottom
    let spawn = { col: col ?? 10, row: row ?? 15 }
    if (this.doorwayTiles.length > 0 && col === undefined) {
      spawn = this.doorwayTiles[0]
    }
  ...
  movePlayer(dir: Direction | null): void {
  ...
  rebuildFromLayout(layout: OfficeLayout, shift?: { col: number; row: number }): void {
    ...
    for (const ch of this.characters.values()) {
      if (ch.isCat || ch.isLobster || ch.isSystemRole || ch.isPlayer) continue
  ...
  update(dt: number): void {
    ...
    for (const ch of this.characters.values()) {
      ...
      if (ch.systemRoleType === 'gateway_sre') {
        ...
        updatePlayer(ch, dt, this.playerMoveDir, this.walkableTiles, this.tileMap, this.blockedTiles)
      } else {
